...
if ($_POST)
{
	...
	$i = 0;
	if ( !$host_name ) {
		$error[$i] = "<div class='error'> <br>You forgot to give a contest host name.</div>";
		$i++;
		$failed=true;
	}
	if ( !$contest_name ) {
		$error[$i] = "<div class='error'><br> You forgot to give the contest a name.</div>";
		$i++;
		$failed=true;
	}
	if ( !$freeze_hour ) {
		$error[$i] = "<div class='error'><br> You forgot to give the contest a freeze hour.</div>";
		$i++;
		$failed=true;
	}
	if ( !$end_hour ) {
		$error[$i] = "<div class='error'><br> You forgot to give the contest an end hour.</div>";
		$i++;
		$failed=true;
	}
/*	if (!date_validate($contest_month, $contest_day, $contest_year)) {
	...
	if ( !$base_directory ) {
		$error[$i] = "<div class='error'><br>You forgot to give the contest a base directory.</div>";
		$i++;
		$failed=true;
	}
	if($failed){
...
#		echo mysql_num_rows($contest_exists);
	...
	if (mysql_num_rows($contest_exists) > 0) {
		...
		if (!$delete) {
			echo "<div class='error'> Error!  Contest";
			echo "creation failed.  Contact administrator.</div>";
		}
	}
